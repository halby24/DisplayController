...
## 2025/03/04 - CI/CD設定の導入

### 背景
- プロジェクトが実装完了し、リリース準備段階に入っている
- 現状では手動でビルドとテストを行っている
- リリースプロセスが手動で行われており、ミスの可能性がある
- 継続的な開発とリリースのためのCI/CD環境が必要

### 決定事項
1. GitHub Actionsを使用したCI/CD設定の導入
   - Windows環境でのビルドとテスト自動化
   - タグプッシュ時の自動リリース作成
   - ビルド成果物の自動アップロード

2. ワークフロー設計
   - プッシュ時とプルリクエスト時のビルドとテスト
   - タグプッシュ時のリリース作成
   - vcpkgを使用した依存関係管理
   - CMakeによるビルド設定
...
### 実装方針
1. `.github/workflows/ci.yml`ファイルの作成
   - ビルドとテストのジョブ設定
   - リリース作成のジョブ設定
   - アーティファクト管理の設定
